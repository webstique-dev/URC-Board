...
export default function BoardView() {
  ...
  // Live updates from other users on this board
  ...
  const onDragEnd = async (result) => {
    ...
    const movedCard = sourceList.cards.find((c) => c._id === draggableId);
  ...
  return (
    <div className="h-[calc(100vh-56px)] flex flex-col">
      <div className="px-6 py-4 border-b border-slate-200 bg-white flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold text-ink">{board.title}</h1>
          ...
          <div className="flex -space-x-2">
            {board.members?.slice(0, 6).map((m) => (
              <span
                key={m.user._id}
                title={m.user.name}
                className="w-7 h-7 rounded-full border-2 border-white flex items-center justify-center text-[10px] text-white font-medium"
        ...
        <div className="flex-1 overflow-x-auto px-6 py-5">
          <div className="flex gap-4 h-full items-start">
            ...
            <div className="w-72 shrink-0">
              {addingList ? (
                <form onSubmit={addList} className="bg-white border border-slate-200 rounded-xl p-3">
                  <input
                    autoFocus
                    value={newListTitle}
                    ...
                    className="w-full text-sm rounded-lg border border-slate-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-accent/40"
                  />
                  <div className="flex gap-2 mt-2">
                    <button className="text-sm bg-accent text-white rounded-lg px-3 py-1.5">Add list</button>
                    <button type="button" onClick={() => setAddingList(false)} className="text-sm text-slate-500 px-2">
                      Cancel
                    </button>
                  </div>
                </form>
                ...
                <button
                  ...
                  className="w-full text-left text-sm text-slate-500 hover:bg-slate-100 rounded-xl px-3 py-2.5 border border-dashed border-slate-300"
                >
                  + Add another list
                </button>
              )}
